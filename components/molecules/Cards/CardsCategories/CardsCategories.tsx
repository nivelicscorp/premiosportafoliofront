...
const CardsCategories = ({
  ...
  return (
    <div className={styles?.cardCategories + ' ' + (variant ? styles?.modal : '')}>
      <div className={styles?.cardCategories__top + ' ' + checkCategoryCard(title)} >
        {variant ?
          <div className={styles?.cardCategories__imgModal + ' ' + checkCategoryCard(title)}>
            <Image
              loader={myLoader}
              src='/img/categories/hand-mobile.png'
              alt={icon.alt}
              title={icon.title}
              height={147}
              width={142}
        ...
        <svg className={styles?.colorCategory} xmlns="http://www.w3.org/2000/svg" width="314" height="22" viewBox="0 0 314 22">
          <linearGradient id="linear-gradient" x1="0.601" y1="0.601" x2="0.429" y2="0.461" gradientUnits="objectBoundingBox">
          ...
          <g id="Enmascarar_grupo_62315" data-name="Enmascarar grupo 62315" transform="translate(-616.597 -185.15)" clip-path="url(#clip-path)">
            <path d="M0,0,677.342,67.079,1806.333,178.887H0Z" transform="matrix(0.998, -0.07, 0.07, 0.998, -150, 172.696)" fill="url(#linear-gradient)"/>
            <path className={styles?.category} d="M0,0,1806.333,160.288H0Z" transform="matrix(0.998, -0.07, 0.07, 0.998, -150, 184.639)" fill="#0c95ae"/>
          </g>
        </svg>
      </div>
      <div
      ...
      />
      {
        ...
        <Link href={'/registro'} passHref>
          <a className={styles?.cardCategories__postulate}>
            Postular 
            <svg id='a' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 33.9 40' width={20} height={20} >
              <path
                fill='#fff'
